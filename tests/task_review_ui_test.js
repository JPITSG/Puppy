...
(async () => {
  ...
  assert.ok(nodes["#tr-diff"].children.some(line => line.classes.has("add")), "the diff's lines fill its inner block");
  assert.equal(nodes[".task-review-diff"].classes.has("hidden"), false);

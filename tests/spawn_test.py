...
failure and no-result paths, timeout and cancel kills), request validation,
node resolution, the turn-bound bridge dispatch with its identity checks and
sweeper reaping, one real Unix-socket round trip, and the HTTP job routes.
...
from puppy import config, db, drivers, runner, spawn_agent, spawn_exec  # noqa: E402
...
def request_for(mode, cwd, timeout_s=60):
...
async def test_one_shot_paths(cwd):
    ...
    job = spawn_exec.manager().start_job(
        request_for("hang", cwd, timeout_s=2), ("remote",))
    running_text = spawn_exec.job_text(job.payload(), "testnode")
    assert "running for" in running_text and job.id in running_text
...
async def test_validation(cwd):
    ...
    assert prepared["timeout_s"] == spawn_exec.DEFAULT_TIMEOUT_S
    ...
    assert spawn_exec.resolve_target("nas.lan")["bid"] >= 1
    try:
